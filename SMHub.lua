...
local function startFly()
    ...
    flyConnection = RunService.Heartbeat:Connect(function(dt)
        ...
        if heldKeys[Enum.KeyCode.W] then moveDir += camCF.LookVector end
...
local function makeSlider(parent, labelText, yPos, min, max, default, displayMult, callback)
    ...
    local function fmt(v) return tostring(math.floor(v * displayMult)) end
...
flyContent.Size                   = UDim2.new(1, 0, 0, FLY_CANVAS)
flyContent.BackgroundTransparency = 1
flyContent.Visible                = false
flyContent.Parent                 = scrollFrame
...
local flyToggleSync = makeToggle(flyContent, "Enable Fly", 28, false, function(v)
    SETTINGS.Fly = v
